...
public class CallBeaconStructure extends Structure {
    public static final Codec<CallBeaconStructure> CODEC = RecordCodecBuilder.<CallBeaconStructure>mapCodec(instance ->
            instance.group(CallBeaconStructure.settingsCodec(instance),
            ...
            ).apply(instance, CallBeaconStructure::new)).codec();
    ...
    public CallBeaconStructure(StructureSettings config,
    ...
    public Optional<GenerationStub> findGenerationPoint(GenerationContext pContext) {
        ...
        int fz = pos2.getZ();
        while (searching) {
            if ((blockReader.getBlock(pos2.getY() + 1).is(BlockTags.REPLACEABLE) && blockReader.getBlock(pos2.getY() + 1).is(BlockTags.REPLACEABLE) && !blockReader.getBlock(pos2.getY() - 1).is(BlockTags.REPLACEABLE))) {
        ...
        return SetRotJigsawPlacement.addPieces(pContext, this.startPool, this.startJigsawName, this.size, pos2.above().above(), false, this.projectStartToHeightmap, this.maxDistanceFromCenter,rot);
    }

    public StructureType<?> type() {
        return ModStructures.CALL_BEACON.get();
    }
}
